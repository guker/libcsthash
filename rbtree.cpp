...
void rbtree_delete_node(util_rbtree_node_t * p){
    if ( p != NULL ){
        rbtree_delete_node(p->left) ;
        rbtree_delete_node(p->right) ;
        delete p;
    }
}

void util_rbtree_clear(util_rbtree_t *rbtree){
    rbtree_delete_node(rbtree->root) ;
    rbtree->root = NULL ;
}
